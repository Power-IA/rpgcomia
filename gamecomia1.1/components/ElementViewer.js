class ElementViewer {
    ...
    toggleElement(element) {
        if (this.selectedElements.has(element.id)) {
            ...
            // Se for uma era, remover qualquer outra era selecionada
            if (element.tipo === 'era') {
                // Encontrar e remover outras eras
                const selectedElementsArray = Array.from(this.selectedElements);
                for (const elementId of selectedElementsArray) {
                    const el = window.ElementSystem.getElementById(elementId);
                    if (el && el.tipo === 'era') {
                        this.selectedElements.delete(elementId);
                        this.removeSelectedElement(elementId);
                    }
                }
            }
    ...
    addSelectedElement(element) {
        ...
        // Adicionar classe específica para diálogos ou eras
        if (element.tipo === 'dialogo') {
            selectedElement.classList.add('dialogo');
        } else if (element.tipo === 'era') {
            selectedElement.classList.add('era');
        }
        ...
        // Adicionar badge para eras
        if (element.tipo === 'era') {
            elementContent = `<span class="era-badge">Era</span>` + elementContent;
        }
